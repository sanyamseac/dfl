...
export const actions: Actions = {
    register: async (event) => {
        ...
        if (!password || !validatePassword(password)) return fail(400, { message: 'Invalid Password' })
        ...
        try {
            ...
            if (existingUser.length > 0)
                return fail(400, { message: 'Email already registered. Please login or contact support.' })
            ...
            if (check.length > 0)
                return fail(400, { message: 'Email already registered. Please verify your email or try another.' })

            await db
                .insert(table.unverified)
                .values({ 
                    id: userId,
                    email, 
                    firstname,
        ...
        } catch (err) {return logError(err as Error)}
        return { success: true, message: 'Registration successful! Please verify your email by clicking on the link sent to proceed.'}
    },
